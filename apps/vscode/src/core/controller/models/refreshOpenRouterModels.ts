...
async function fetchAndCacheModels(
	...
	let finalModels = appendClineStealthModels(models);

	// Append models from user config file (~/.cline/config.yaml)
	try {
		const clineConfig = await loadClineConfig();
		const configModels = configModelsToModelInfo(clineConfig.models);
		if (Object.keys(configModels).length > 0) {
			finalModels = appendConfigModels(finalModels, configModels);
			Logger.log(
				`[Cline Config] Merged ${Object.keys(configModels).length} models from config.yaml`,
			);
		}
	} catch (error) {
		Logger.error("[Cline Config] Error merging config models:", error);
	}
...
/**
 * Append models from the user's config.yaml to the model list.
 * Config models override API-fetched models if they share the same ID,
 * allowing users to customize model metadata.
 */
export function appendConfigModels(
	currentModels: Record<string, ModelInfo>,
	configModels: Record<string, ModelInfo>,
): Record<string, ModelInfo> {
	const cloned = { ...currentModels };
	for (const [modelId, modelInfo] of Object.entries(configModels)) {
		// Config models are merged in - they can override existing entries
		// or add new models not available from the OpenRouter API
		cloned[modelId] = modelInfo;
	}
	return cloned;
}
